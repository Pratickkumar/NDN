...
Interest::wireEncode(EncodingImpl<TAG>& encoder) const
{
  ...
  if (getHopCount()) {
    totalLength += prependBinaryBlock(encoder, tlv::HopCount, {*hopcount});
  }
...
Interest::wireDecode(const Block& wire)
{
  ...
  sojourn = DEFAULT_INTEREST_LIFETIME;
  m_hopLimit.reset();
  hopcount.reset();
  m_parameters.clear();
  ...
  for (++element; element != m_wire.elements_end(); ++element) {
    switch (element->type()) {
      ...
      case tlv::InterestLifetime: {
        ...
        sojourn = time::milliseconds(readNonNegativeInteger(*element));
        lastElement = 6;
        break;
      }
      case tlv::HopLimit: {
      ...
      case tlv::HopCount: {
        ...
        if (element->value_size() != 1) {
          NDN_THROW(Error("HopCount element is malformed"));
        }
        hopcount = *element->value();
        lastElement = 7;
        break;
      }
...
Interest&
Interest::setSojourn(time::milliseconds Sojourn)
{
  if (Sojourn < 0_ms) {
    NDN_THROW(std::invalid_argument("Sojourn must be >= 0"));
  }
...
Interest&
Interest::setHopCount(optional<uint8_t> hopCount)
{
